        ...
        let res = _fetch(this.url.toString(), {
          ...
          if (res$1.ok) {
            ...
            if (_this.method !== "HEAD") {
              ...
              if (body === "") {
              } else if (_this.headers.get("Accept") === "text/csv") data2 = body;
              else if (_this.headers.get("Accept") && ((_this$headers$get = _this.headers.get("Accept")) === null || _this$headers$get === void 0 ? void 0 : _this$headers$get.includes("application/vnd.pgrst.plan+text"))) data2 = body;
              else data2 = JSON.parse(body);
            }
            const countHeader = (_this$headers$get2 = _this.headers.get("Prefer")) === null || _this$headers$get2 === void 0 ? void 0 : _this$headers$get2.match(/count=(exact|planned|estimated)/);
            const contentRange = (_res$headers$get = res$1.headers.get("content-range")) === null || _res$headers$get === void 0 ? void 0 : _res$headers$get.split("/");
            if (countHeader && contentRange && contentRange.length > 1) count = parseInt(contentRange[1]);
            if (_this.isMaybeSingle && Array.isArray(data2)) if (data2.length > 1) {
              error = {
                code: "PGRST116",
...
function createFetchClient(options) {
  ...
  return {
    async request({
      ...
      const data2 = isJson && text ? JSON.parse(text) : text;
      if (!res.ok) {
      ...
      */
      async createSignedUrl(path, expiresIn, options) {
        ...
        return _this8.handleOperation(async () => {
          ...
          const returnedPath = hasTransform && data2.signedURL.includes("/object/sign/") ? data2.signedURL.replace("/object/sign/", "/render/image/sign/") : data2.signedURL;
      ...
      */
      async createSignedUrls(paths, expiresIn, options) {
        ...
        return _this9.handleOperation(async () => {
          ...
          const downloadQueryParam = (options === null || options === void 0 ? void 0 : options.download) ? `&download=${options.download === true ? "" : options.download}` : "";
          return data2.map((datum) => _objectSpread22(_objectSpread22({}, datum), {}, { signedUrl: datum.signedURL ? encodeURI(`${_this9.url}${datum.signedURL}${downloadQueryParam}`) : null }));
        });
      }
      /**
    ...
    var GoTrueClient = class _GoTrueClient {
      ...
      async linkIdentityOAuth(credentials) {
        ...
        try {
          ...
          return this._returnResult({
            data: { provider: credentials.provider, url: data2 === null || data2 === void 0 ? void 0 : data2.url },
            error: null
          });
        } catch (error) {
          if ((0, errors_1.isAuthError)(error)) {
    ...
    SupabaseClient = class {
      ...
      */
      ...
      async _getAccessToken() {
        ...
        return (_data$session$access_ = (_data$session = data2.session) === null || _data$session === void 0 ? void 0 : _data$session.access_token) !== null && _data$session$access_ !== void 0 ? _data$session$access_ : _this.supabaseKey;
      }
...
var useStore2, useStore_default;
var init_useStore = __esm({
  "src/store/useStore.js"() {
    init_esm();
    init_supabase();
    init_api();
    useStore2 = create((set, get2) => ({
      // Auth state
      user: null,
      session: null,
      authLoading: true,
      ...
      chatStreamingText: "",
      chatSessionId: null,
      addChatMessage: (message) => set((state2) => ({
        chatMessages: [...state2.chatMessages, message]
      })),
      ...
      toggleDarkMode: () => set((state2) => {
        const next = !state2.darkMode;
        try {
          window.localStorage?.setItem("cbe_theme", next ? "dark" : "light");
        } catch {
        }
      ...
      persona: (() => {
        ...
        try {
          return window.localStorage?.getItem("cbe_persona") || null;
        } catch {
          return null;
        }
      })(),
      setPersona: (persona) => {
        try {
          window.localStorage?.setItem("cbe_persona", persona || "");
        } catch {
        }
        set({ persona });
      },
      ...
      activeDepthLevel: (() => {
        ...
        try {
          return window.localStorage?.getItem("cbe_depth_level") || "beginner";
        } catch {
          return "beginner";
        }
      })(),
      setActiveDepthLevel: (level) => {
        try {
          window.localStorage?.setItem("cbe_depth_level", level);
        } catch {
        }
      ...
      pipelineStatus: null,
      // null | 'pending' | 'processing' | 'stage_N' | 'complete' | 'failed' | 'cancelled'
      pipelineProgress: null,
      // { stage, total_stages, message }
      ...
      cancelPipeline: async () => {
        const { projectId, _sseCleanup } = get2();
      ...
      markConceptExplored: (id) => set((state2) => {
        const next = new Set(state2.exploredConcepts);
        next.add(id);
      ...
      userState: null,
      setUserState: (state2) => set({ userState: state2 }),
      // Guided tour mode
      guidedMode: false,
      guidedPosition: 0,
      explorationPath: [],
      ...
      advanceGuided: () => {
        const { guidedPosition, explorationPath, markConceptExplored } = get2();
      ...
      retreatGuided: () => {
        const { guidedPosition, explorationPath } = get2();
      ...
      addQuizAnswer: (answer) => set((state2) => ({ quizAnswers: [...state2.quizAnswers, answer] })),
      resetQuizGate: () => set({
        quizGateActive: false,
        quizGateQuestions: [],
        quizGateType: null,
      ...
      subConceptsCache: {},
      subConceptsLoading: /* @__PURE__ */ new Set(),
      subConceptsReadyKeys: /* @__PURE__ */ new Set(),
      ...
      fetchSubConcepts: async (conceptKey) => {
        const state2 = get2();
        if (state2.subConceptsLoading.has(conceptKey)) return;
        if (state2.expansions[conceptKey]) return;
        const cached = state2.subConceptsCache[conceptKey];
        if (cached?.ready && cached.subConcepts?.length) {
          get2().expandConcept(conceptKey, cached.subConcepts, cached.subEdges || []);
          return;
        }
        const loading = new Set(state2.subConceptsLoading);
        loading.add(conceptKey);
        set({ subConceptsLoading: loading });
        try {
          const res = await fetch(`${API_BASE}/api/pipeline/${state2.projectId}/sub-concepts/${conceptKey}`);
          ...
          set((s) => {
            ...
            return {
              subConceptsCache: {
                ...s.subConceptsCache,
                [conceptKey]: { subConcepts: data2.sub_concepts || [], subEdges: data2.sub_edges || [], ready: data2.ready }
              },
              subConceptsLoading: nextLoading
            };
          });
          if (data2.ready && data2.sub_concepts?.length) {
            get2().expandConcept(conceptKey, data2.sub_concepts, data2.sub_edges || []);
          }
        } catch {
          const nextLoading = new Set(get2().subConceptsLoading);
          nextLoading.delete(conceptKey);
      ...
      applyGraphOperations: (graphOps) => {
        ...
        if (auto_collapse?.length) {
          for (const conceptId of auto_collapse) {
            if (state2.expansions[conceptId]) {
              get2().collapseConcept(conceptId);
            }
          }
        }
        for (const op of operations) {
          if (op.type === "expand_concept" && op.parent_concept_id && op.sub_concepts?.length) {
            const parentExists = get2().concepts.some((c) => c.id === op.parent_concept_id);
            if (!parentExists) continue;
            if (get2().expansions[op.parent_concept_id]) continue;
            get2().expandConcept(op.parent_concept_id, op.sub_concepts, op.sub_edges || []);
          } else if (op.type === "highlight_path" && op.path?.length >= 2) {
            get2().highlightPathNodes(op.path, op.path_label || "");
            ...
            const sourceExists = get2().concepts.some((c) => c.id === op.source);
            const targetExists = get2().concepts.some((c) => c.id === op.target);
        ...
        const visibleCount = get2().concepts.length;
        if (visibleCount > 35) {
          const history = get2().expansionHistory;
          if (history.length > 0) {
            const oldest = history[0];
            get2().collapseConcept(oldest.conceptId);
            get2().showToast(`Collapsed ${oldest.name} to keep graph readable`);
          }
        }
      },
      expandConcept: (parentId, subConcepts, subEdges) => {
        const state2 = get2();
        const parent = state2.concepts.find((c) => c.id === parentId);
        if (!parent) return;
        ...
        const nextExpandedIds = new Set(state2.expandedNodeIds);
      ...
      collapseConcept: (parentId) => {
        const state2 = get2();
        const expansion = state2.expansions[parentId];
        if (!expansion) return;
        const childIds = new Set(expansion.subConcepts);
        const nextExpandedIds = new Set(state2.expandedNodeIds);
        childIds.forEach((id) => nextExpandedIds.delete(id));
        const nextExpansions = { ...state2.expansions };
        delete nextExpansions[parentId];
        set({
          concepts: state2.concepts.filter((c) => !childIds.has(c.id)),
          conceptEdges: state2.conceptEdges.filter(
            (e) => !childIds.has(e.source) && !childIds.has(e.target) && !(e.source === parentId && childIds.has(e.target))
          ),
          expansions: nextExpansions,
          expandedNodeIds: nextExpandedIds,
          expansionHistory: state2.expansionHistory.filter((h) => h.conceptId !== parentId)
        });
      },
      collapseAll: () => {
        const state2 = get2();
        const allExpansionNodeIds = /* @__PURE__ */ new Set();
        for (const exp of Object.values(state2.expansions)) {
          exp.subConcepts.forEach((id) => allExpansionNodeIds.add(id));
        }
        set({
          concepts: state2.concepts.filter((c) => !allExpansionNodeIds.has(c.id)),
          conceptEdges: state2.conceptEdges.filter(
            (e) => !allExpansionNodeIds.has(e.source) && !allExpansionNodeIds.has(e.target) && !e._temporary
          ),
          expansions: {},
          expandedNodeIds: /* @__PURE__ */ new Set(),
          expansionHistory: [],
      ...
      highlightPathNodes: (nodeIds, label) => {
        ...
        setTimeout(() => {
          if (get2().highlightedPath?.nodeIds === nodeIds) {
      ...
      getExpansionState: () => {
        const state2 = get2();
        return {
          expanded_concepts: Object.keys(state2.expansions),
          visible_node_count: state2.concepts.length
        };
      },
      // Insights
      insights: [],
      ...
      originContext: null,
      // 'self_built' | 'ai_built' | 'someone_else'
      ...
      // Helpers
      getConceptById: (id) => get2().concepts.find((c) => c.id === id),
      getFileById: (id) => get2().files.find((f) => f.id === id),
      getFilesByConcept: (conceptId) => get2().files.filter((f) => f.conceptId === conceptId),
      getConnectedConcepts: (conceptId) => {
        const edges = get2().conceptEdges;
    ...
    useStore_default = useStore2;
  }
});

// src/lib/loadProject.js
var loadProject_exports = {};
__export(loadProject_exports, {
  fetchAndLoadProject: () => fetchAndLoadProject,
  loadProjectData: () => loadProjectData
});
function loadQuizStats(quizState) {
  if (!quizState?.length) return;
  const store = useStore_default.getState();
  const stats = {};
...
async function fetchAndLoadProject(projectId) {
  try {
    const headers = {};
    ...
    const data2 = await res.json();
    return loadProjectData(data2, projectId);
  } catch (err) {
    console.error("Failed to load project data:", err);
    return null;
  }
}
function loadProjectData(data2, projectId) {
  const store = useStore_default.getState();
  store.resetProject();
  const concepts = (data2.concepts || []).map((c) => ({
    id: c.concept_key,
    name: c.name,
    color: c.color,
    ...
    fileIds: (data2.files || []).filter((f) => f.concept_id === c.concept_key).map((f) => f.path)
  }));
  const files = (data2.files || []).map((f) => ({
    id: f.path,
    name: f.name,
  ...
  const conceptEdges = (data2.edges || []).map((e) => ({
  ...
  if (data2.insights) {
    store.setInsights(data2.insights);
  }
  if (data2.userState) {
    store.setUserState(data2.userState);
    const path = data2.userState.exploration_path;
    if (path?.length) {
  ...
  if (data2.quizState) {
    loadQuizStats(data2.quizState);
  }
  store.setProjectId(projectId);
  if (data2.project) {
    store.setProjectMeta(data2.project);
  }
  if (data2.sub_concepts?.length) {
    const grouped = {};
    data2.sub_concepts.forEach((sc) => {
    ...
    const edgesByParent = {};
    (data2.sub_concept_edges || []).forEach((se) => {
  ...
  if (data2.chatMessages?.length) {
    const mapped = data2.chatMessages.map((msg) => ({
      role: msg.role,
      content: msg.content,
    ...
    const lastMsg = data2.chatMessages[data2.chatMessages.length - 1];
...
// codex-zoom-store-test-input.mjs
globalThis.window = {
  localStorage: {
    getItem: () => null,
    setItem: () => {
...
var { default: useStore3 } = await Promise.resolve().then(() => (init_useStore(), useStore_exports));
var { loadProjectData: loadProjectData2 } = await Promise.resolve().then(() => (init_loadProject(), loadProject_exports));
var data = {
...
loadProjectData2(data, "zoom-test");
var state = useStore3.getState();
